...
import { getBranches, branchLogin, forgotPassword } from '../api';
...
export default function BranchLogin() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const { token, user } = await branchLogin(
        branchId,
        username,
        password
      );
    ...
    <div className="login-wrap">
      <div className="login-box">
        <div className="login-box-header">
          <div className="tag">Branch Sign In</div>
          <h2>{branch ? branch.name : 'Loading…'}</h2>
        </div>
